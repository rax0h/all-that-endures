...
class AgencyState:
 ...
 def assess(self,world,p,attachment=None,dependents=None):
  ...
  m=MotiveState(max(0.,min(1.,q.scarcity+max(0.,(5-h.food)/10))),max(0.,min(1.,world.cells[(world.settlements[p.settlement].x,world.settlements[p.settlement].y)].hazard*(1-h.preparedness)+p.fear)),max(0.,1-attachment),max(0.,min(1.,1-p.wealth/120.)),p.curiosity,max(0.,min(1.,p.age/80))*p.attachment,min(1.,dependents*.18+p.grief*.2),max(0.,min(1.,.65-p.wealth/250.))*(.5+.5*(1-p.inhibition)))
...
def agency_step(world,rng):
 ...
 for p in sorted((x for x in world.people.values() if x.alive and x.age>=16),key=lambda x:x.id):
  ...
  if path is not None:
   before=world.advancement.rank(p.id);ability=int(rr.random()*20)%20
   meaningful=strength*(.45+.55*p.curiosity)*(1.15 if action in ('prepare','learn','teach') else .75)
   reflection=(p.curiosity*.7+p.attachment*.3)*(1.0 if action in ('learn','teach','socialize') else .2)
   world.advancement.practice(p.id,ability,meaningful*.04,reflection)
   after=world.advancement.rank(p.id)
   if after>before:
    p.rank=after;Layer,Ref=layer_ref();world.emit('rank_advanced',Layer.REALITY,actors=(Ref('person',p.id),),location=Ref('settlement',p.settlement),from_rank=before,to_rank=after)
