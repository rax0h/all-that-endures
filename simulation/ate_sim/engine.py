...
from .species import species, compose_biology
...
class Simulation:
    ...
    def bio(self,p): return compose_biology(p.species,profile(p.rank))
    def _weather(self):
    ...
    def _production(self):
        for sid,s in self.w.settlements.items():
            people=[self.w.people[p] for h in s.households for p in self.w.households[h].members if self.w.people[p].alive]; c=self.w.cells[(s.x,s.y)]
            labor=sum(self.bio(p)["endurance"]*p.health for p in people); food_need=sum(self.bio(p)["food_need"] for p in people)
    ...
    def _people(self):
        for pid,p in list(self.w.people.items()):
            ...
            p.age+=1;s=self.w.settlements[p.settlement];r=self.rng.stream("life",self.w.year,pid);sp=species(p.species);mortality=annual_mortality(int(p.age/sp.baseline_longevity),p.rank,s._scarcity)
            if r.random()<mortality:
                p.alive=False;e=self.w.emit("death",Layer.REALITY,(Ref("person",pid),),Ref("settlement",p.settlement),age=p.age,rank=p.rank,species=p.species);h=self.w.households[p.household]
    ...
    def _demography(self):
        for hid,h in list(self.w.households.items()):
            ...
            living=[self.w.people[i] for i in h.members if self.w.people[i].alive];adults=[p for p in living if 18<=p.age<=42*species(p.species).baseline_longevity];s=self.w.settlements[h.settlement];r=self.rng.stream("birth",self.w.year,hid);fert=sum(species(p.species).fertility for p in adults)/max(1,len(adults));chance=.055*fert*min(1,len(adults)/2)*(1-.75*s._scarcity)
            if len(living)<8 and r.random()<chance and living:
                ...
                parents=sorted(base,key=lambda p:p.age,reverse=True)[:2];sp=parents[0].species if len({p.species for p in parents})==1 else r.choice([p.species for p in parents]);p=Person(pid,self.w.year,h.settlement,hid,age=0,temperament=inh("temperament"),attachment=inh("attachment"),curiosity=inh("curiosity"),inhibition=inh("inhibition"),species=sp);self.w.people[pid]=p;h.members.append(pid);self.w.emit("birth",Layer.REALITY,(Ref("person",pid),),Ref("settlement",h.settlement),household=hid,species=sp)
        ...
        for sid,s in self.w.settlements.items():
            ...
            if r.random()<.012*c.hazard:
                ...
                for p in residents:
                    ...
                    if self.rng.stream("surge_person",self.w.year,p.id).random()<severity*.12/resilience:p.alive=False;casualties+=1;self.w.emit("death",Layer.REALITY,(Ref("person",p.id),),Ref("settlement",sid),(attack.id,),cause="monster_surge",rank=p.rank,species=p.species)
