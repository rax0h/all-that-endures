...
class Simulation:
 ...
 def run(self,years):
  ...
  return self.w
 def step(self):
  self.w.year+=1; self._weather(); self._production(); self._people(); household_step(self.w,self.rng); self._demography(); self._pressure(); agency_step(self.w,self.rng); cultural_step(self.w,self.w.culture,self.rng); civilization_step(self.w,self.rng); development_step(self.w,self.rng); self._memory()
 def _weather(self):
