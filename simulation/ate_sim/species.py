...
# Fertility remains neutral until reproductive differences are canonically specified. Do not let provisional species flavor decide demographic survival.
...
def reproductive_compatibility(a_key,b_key):
 """Compatibility contract. It defaults neutral until cross-species reproductive biology is ratified.
 Social attraction and culture must never be inferred from this value.
 """
 species(a_key);species(b_key)
 return 1.0
def reproductive_span(species_key):
 """Reproductive opportunity grows with lifespan, but annual fecundity is normalized so longevity alone does not dominate evolution."""
 return 34.*species(species_key).baseline_longevity
def inherit_species(a_key,b_key,rng):
 """No invented hybrid species: a child takes one parental species until inheritance canon is specified."""
