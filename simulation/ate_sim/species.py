...
def reproductive_compatibility(a_key,b_key):
 """Provisional embodied compatibility, not culture or attraction.
 Same-species reproduction is baseline. Cross-species reproduction remains possible,
 but large differences in body scale and aquatic physiology reduce biological compatibility.
 """
 if a_key==b_key:return 1.0
 a,b=species(a_key),species(b_key)
 scale=abs(a.mass-b.mass)+abs(a.stature-b.stature)
 aquatic=abs(a.aquatic-b.aquatic)
 return max(.18,min(.58,.58-.16*scale-.28*aquatic))
def inherit_species(a_key,b_key,rng):
 """No invented hybrid species: a child takes one parental species when compatible."""
