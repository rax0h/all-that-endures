...
def _observe_preexisting_essence(w,rr,p,sid,founded,essence=None):
 ...
 found=w.emit('essence_resource_found',Layer.REALITY,(Ref('person',p.id),),Ref('settlement',sid),(founded.id,),
              essence=essence,provenance='pre-simulation magical civilization',observation_boundary=True)
 resource=w.magic_resources.create('essence',essence,ESSENCES[essence]['rarity'],0,sid,'person',p.id,found.id)
 absorbed=w.emit('essence_absorbed',Layer.REALITY,(Ref('person',p.id),),Ref('settlement',sid),(found.id,),
...
def _seed_ranked_founder(w,rr,p,sid,founded,target_rank):
 """Observe a real preexisting complete path at the year-zero boundary.

 Missing essences and stones are represented as physical pre-observation
 resources with provenance and consumption events. Iron/Bronze/Silver history
 is then reconstructed through the ordinary advancement event machinery.
 """
 ...
 while len(path.abilities)<20:
  ...
  sf=w.emit('awakening_stone_found',Layer.REALITY,(Ref('person',p.id),),Ref('settlement',sid),(founded.id,),
            stone=skey,provenance='pre-simulation magical civilization',observation_boundary=True)
  resource=w.magic_resources.create('awakening_stone',skey,AWAKENING_STONES[skey]['rarity'],0,sid,'person',p.id,sf.id)
  before=p.rank
  used=w.emit('awakening_stone_used',Layer.REALITY,(Ref('person',p.id),),Ref('settlement',sid),(sf.id,),
              resource=resource.id,stone=skey,target_essence=None,observation_boundary=True,preexisting=True)
  ability=w.advancement.awaken_skill(p.id,skey,0,_context(p,sid),used.id)
 ...
 for tier in range(2,max(1,target_rank)+1):
  before=p.rank
  exposure=20. if tier==2 else 40.
  for i in range(len(path.abilities)):
   practice_ability(w,p,i,exposure,.5,context='pre-observation magical career',body_rank=before)
...
def _seed_essence_for_person(w,rr,p,sid,founded):
 ...
 if rr.random()<.12:
  skey=rr.choice(STONE_IDS);sf=w.emit('awakening_stone_found',Layer.REALITY,(Ref('person',p.id),),Ref('settlement',sid),(founded.id,),stone=skey,provenance='pre-simulation magical civilization',observation_boundary=True);w.magic_resources.create('awakening_stone',skey,AWAKENING_STONES[skey]['rarity'],0,sid,'person',p.id,sf.id)
...
def generate_world(seed:int,width=24,height=18,settlements=5):
 ...
 # Year zero observes an established civilization, so a substantial fraction
 # of the already-magical adult population must already be working ranked paths.
 # This is initial historical state only; later ranks must be replenished by the
 # live civilization rather than maintained to this share.
 for sid in sorted(w.settlements):
  users=[p for p in w.current_people() if p.alive and p.age>=18 and p.settlement==sid and w.advancement.essence_user(p.id)]
  ...
  ranked=sorted(users,key=career_score,reverse=True)[:ranked_count]
  for p in ranked:
