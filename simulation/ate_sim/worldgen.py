...
def generate_world(seed:int,width=24,height=18,settlements=5):
    ...
    for c in chosen:
        ...
        for _ in range(rr.randint(5,9)):
            hid=w.next_household; w.next_household+=1; h=Household(hid,sid,wealth=rr.uniform(15,90),food=rr.uniform(8,20),preparedness=rr.uniform(.05,.3),lineage=f"Line-{sid}-{hid}"); w.households[hid]=h; s.households.append(hid)
            for _ in range(rr.randint(2,6)):
                pid=w.next_person; w.next_person+=1; age=rr.randint(0,45); p=Person(pid,-age,sid,hid,age=age,wealth=h.wealth/max(1,len(h.members)+1),temperament=rr.random(),attachment=rr.random(),curiosity=rr.random(),inhibition=rr.random()); w.people[pid]=p; h.members.append(pid)
        w.emit("settlement_founded",Layer.REALITY,location=Ref("settlement",sid),fertility=c.fertility)
    return w
