...
def _seed_essence_for_person(w,rr,p,sid,founded):
 ...
 # Some founders retain an unspent stone; it exists as property and can be traded/inherited/used later.
 if rr.random()<.12:
  skey=rr.choice(STONE_IDS);sf=w.emit('awakening_stone_found',Layer.REALITY,(Ref('person',p.id),),Ref('settlement',sid),(found.id,),stone=skey,provenance='founder-era local discovery');w.magic_resources.create('awakening_stone',skey,AWAKENING_STONES[skey]['rarity'],0,sid,'person',p.id,sf.id)
