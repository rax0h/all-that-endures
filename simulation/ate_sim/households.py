...
def partnership_step(world,rng):
    ...
    for sid,people in by_settlement.items():
        ...
        for i,a in enumerate(people):
            ...
            e=world.emit("partnership_formed",Layer.SOCIETY,(Ref("person",a.id),Ref("person",b.id)),Ref("settlement",sid),compatibility=score);world.social.record(a.id,b.id,e.id,trust=.12,attachment=.20,obligation=.08);world.social.partner(a.id,b.id,e.id);paired.update((a.id,b.id))
            if a.household!=b.household and rr.random()<.62:
                source=world.households[a.household];source2=world.households[b.household];parent_households=(source.id,source2.id);nhid=world.next_household;world.next_household+=1;share=max(4.,source.wealth*.12+source2.wealth*.12);source.wealth=max(0.,source.wealth-source.wealth*.12);source2.wealth=max(0.,source2.wealth-source2.wealth*.12);nh=Household(nhid,sid,wealth=share,food=8.,preparedness=max(.05,(source.preparedness+source2.preparedness)/2),lineage=source.lineage);world.households[nhid]=nh;world.settlements[sid].households.append(nhid)
                for p in (a,b):
...
def household_split_step(world,rng):
    for hid,h in list(world.households.items()):
        ...
        movers=sorted(adults,key=lambda p:(-p.age,p.id))[:max(1,len(adults)//3)];nhid=world.next_household;world.next_household+=1;share=h.wealth*.28;h.wealth-=share;nh=Household(nhid,h.settlement,wealth=share,food=max(3.,h.food*.25),preparedness=max(.05,h.preparedness*.8),lineage=h.lineage);world.households[nhid]=nh;world.settlements[h.settlement].households.append(nhid)
        for p in movers:
