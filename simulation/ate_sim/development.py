...
def development_step(world,rng):
    ...
    adults_by_settlement=world.adults_by_settlement(18)
    for sid,s in world.settlements.items():
        ...
        local_assets=[]
        for kind in ('irrigation','road'):local_assets.extend(world.infrastructure.at_settlement(kind,sid))
    ...
    for (a,b),route in sorted(world.trade_routes.items()):
        existing=[x for x in world.infrastructure.at_settlement('road',a) if b in x.settlements]
        ...
        roads=world.infrastructure.at_settlement('road',a)
        if roads:world.settlements[a].roads=max(.03,min(1.,sum(x.condition for x in roads)/len(roads)))
        roads=world.infrastructure.at_settlement('road',b)
