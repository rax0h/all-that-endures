...
def test_magic_society_remote_order_does_not_require_local_stock_exhaustion():
    ...
    buyer=next(p for p in world.current_people() if p.age>=18 and not world.advancement.completed_path(p.id))
    buyer.wealth=1000.;asp=_aspiration(world,buyer);asp.desired_base_essences=3
