...
def test_cpu_benchmark_gate_reports_failure_and_success(monkeypatch,capsys):
    monkeypatch.delenv('ATE_PROFILE_TAIL',raising=False)
    with pytest.raises(SystemExit,match='CPU budget'):
        main(17,1,max_cpu_seconds=1e-12)
    failed=json.loads(capsys.readouterr().out.splitlines()[-1])
    assert failed['performance_passed'] is False
    assert failed['cpu_performance_passed'] is False
    assert failed['simulation_cpu_seconds']>failed['max_cpu_seconds']
    main(17,1,max_cpu_seconds=120)
    passed=json.loads(capsys.readouterr().out.splitlines()[-1])
    assert passed['performance_passed'] is True
    assert passed['cpu_performance_passed'] is True
    assert passed['digest']==failed['digest']
