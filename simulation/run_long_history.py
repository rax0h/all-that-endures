...
def snapshot(world, include_digest=False):
    ...
    martial_schools=[c for c in world.communities.communities.values() if c.active and c.kind=='martial_school'];school_ids={c.id for c in martial_schools};school_members={pid for (pid,cid),strength in world.communities.memberships.items() if cid in school_ids and strength>=.18 and world.people.get(pid) is not None and world.people[pid].alive};school_member_ranks=Counter(world.advancement.rank(pid) for pid in school_members if world.advancement.essence_user(pid))
    recent_rank_advances=Counter(e.data.get('to_rank',0) for e in world.events if e.kind=='rank_advanced' and e.year>world.year-100)
    awakened_counts=Counter();recent_completed_paths=0
    for e in world.events:
        if e.kind!='ability_awakened' or not e.actors:continue
        pid=e.actors[0].id;awakened_counts[pid]+=1
        if awakened_counts[pid]==20 and e.year>world.year-100:recent_completed_paths+=1
    ...
    recent_first_events=[e for e in first_essence_events if e.year>world.year-100]
    recent_first_search_years=sorted(int(e.data.get('search_years',0)) for e in recent_first_events)
    recent_first_ages=sorted(int(e.data.get('age',0)) for e in recent_first_events)
    def pct(values,q):
    ...
    result={
        ...
        'magic_resources_total':len(resources),'magic_resources_by_kind':dict(sorted(resource_kinds.items())),'magic_resources_available':len(available),'magic_resources_available_by_kind':dict(sorted(available_kinds.items())),'magic_resource_purchases':events['magic_resource_purchased'],'magic_resource_transfers':events['magic_resource_transferred'],'essence_absorptions':events['essence_absorbed'],'first_essence_absorptions':len(first_essence_events),'first_essence_absorptions_last_100y':recent_first_essence,'first_essence_search_years_median':pct(first_search_years,.5),'first_essence_search_years_p90':pct(first_search_years,.9),'first_essence_age_median':pct(first_ages,.5),'first_essence_age_p90':pct(first_ages,.9),'recent_first_essence_search_years_median':pct(recent_first_search_years,.5),'recent_first_essence_search_years_p90':pct(recent_first_search_years,.9),'recent_first_essence_age_median':pct(recent_first_ages,.5),'recent_first_essence_age_p90':pct(recent_first_ages,.9),'awakening_stones_used':events['awakening_stone_used'],'abilities_awakened':events['ability_awakened'],
        ...
        'core_institutions':dict(sorted(inst_kinds.items())),'institution_branches':len(world.institutions.branches),'magic_registry_records':len(world.institutions.magic_records),'magic_registry_disclosure':dict(sorted(disclosures.items())),'adventure_notices':len(world.institutions.notices),'adventure_notice_status':dict(sorted(notice_status.items())),'martial_schools':len(martial_schools),'martial_school_members_living':len(school_members),'martial_school_member_ranks':dict(sorted(school_member_ranks.items())),'rank_advances_last_100y':dict(sorted(recent_rank_advances.items())),'completed_paths_last_100y':recent_completed_paths,
    }
